import { View, Text, ScrollView, SafeAreaView, FlatList, Image, Pressable } from 'react-native'
...
const Enrolled = ({navigation}) => {
  ...
  const removeDuplicate=(data)=>{return data.filter((value,index)=>data.indexOf(value)===index)}
  ...
  return (
    ...
    <FlatList
        style={tailwind`h-20`}
        data={removeDuplicate(db.enroll)}
        ...
        <View style={tailwind`py-5 my-2  border-gray-200 rounded-xl bg-white border-b-2`}>
            <View style={tailwind`flex-row gap-2 `}>
                ...
                <View style={tailwind`gap-2`}><Text style={tailwind`text-sm text-gray-400`}>Course</Text><Text style={tailwind`text-semibold  `}>{item.name}</Text><View style={tailwind`flex-row justify-between`}><Text style={tailwind`text-gray-500 bg-[#76c4ef] w-20 px-3 py-1 rounded-xl`}>Enrolled</Text><TrashIcon color={'gray'} onPress={()=>db.setEnroll(db.enroll.filter((elem)=>{return elem.id!=item.id}))} style={tailwind` rounded-full justify-center p-2 items-center`}/></View><Text style={tailwind`text-gray-500`}>{item.duration}</Text></View>
            </View>
       </View>
       </Pressable>
       )}}
