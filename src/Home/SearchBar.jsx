...
const SearchBar = (props) => {
  ...
  useEffect(() => {
    setSearched(() => courses.filter((elem) => elem.name.includes(item)));
  }, [])
    ...
    <SafeAreaView
      ...
      <View style={tailwind `justify-center items-center bg-white rounded-b-[10]`}>
        <View style={tailwind `flex-row items-center p-4 w-full gap-10`}>
          <View style={tailwind `p-2 bg-white rounded-full`}><ChevronLeftIcon
            size={30}
            color={'gray'}
            onPress={() => props.navigation.goBack()}/></View>
          <Text style={tailwind `text-xl font-semibold`}>Search Results :</Text>
        </View>
      </View>
      ...
      <View>{(searched.length > 0)
          ...
          : (
            <View style={tailwind `h-[70%] items-center justify-center`}>
              <View style={tailwind `h-100 w-100`}><Image
                style={{
              height: "100%",
              width: "100%"
            }}
                source={require('../../assets/none.webp')}/></View>
              <Text style={tailwind `absolute bottom-10`}>Nothing Here !!</Text>
            </View>
          )}</View>
