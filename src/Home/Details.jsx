...
const Details = (props) => {
  ...
  return (
    ...
    <View style={tailwind` p-4  `}>
     ...
     <View style={tailwind`mt-8 flex-row gap-3 `}>
       ...
       <View style={tailwind`flex-row gap-2 items-center`}><UserCircleIcon color={'gray'}/><Text style={tailwind`text-gray-500`}>{item.instructor}</Text></View>
     </View>
    </View>
    
    <View style={tailwind`flex-1`}>
    <View style={tailwind`flex-row justify-center mt-2 mb-3 gap-10`}>
     <Pressable onPress={()=>setFocus(false)}>   <Text style={tailwind`text-xl ${focus?`text-gray-500`:`text-blue-500 border-b-2 border-blue-500`}`}>Lessons</Text> </Pressable>
     <Pressable onPress={()=>setFocus(true)}>  <Text style={tailwind`text-xl ${focus?`text-blue-500 border-b-2 border-blue-500`:`text-gray-500`}`}>Description</Text> </Pressable>
     </View>
  {focus?<View style={tailwind`p-4`}><Text>{item.description}</Text></View>:<ScrollView showsVerticalScrollIndicator={false} >
    ...
    <View style={tailwind`bg-white-10 justify-center gap-10  flex-row h-20 p-2`}>
        <View style={tailwind`bg-white h-10 w-20 flex justify-center items-center rounded-xl  `}><Text style={tailwind`text-[#3aaceb] font-semibold`}>{item.price}</Text></View>
